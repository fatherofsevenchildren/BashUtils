...
void printCatWithFlags(FILE *f, Flags flags) {
  char carentchar;
  int countSlash = 1;  // обнуляю с каждой новой непустой строкой в -s
  int countStart = 0;  // ставлю номер строки если для этого был \n
  int countString = 1;  //считаю строки для -n
